...
import { Send, Bot, CheckCircle2, MoreVertical, LayoutGrid, Users, Settings, ArrowLeft, ExternalLink, RotateCcw, Monitor } from 'lucide-react';
import PixelStudio from '@/components/studio/PixelStudio';
import GamifiedHub from '@/components/studio/GamifiedHub';
        ...
        <div className="flex h-screen w-full flex-col bg-zinc-950 text-white font-sans overflow-hidden">
            ...
            <header className="flex h-16 shrink-0 items-center justify-between border-b border-zinc-800 bg-zinc-950 px-6">
                <div className="flex items-center gap-4">
                    ...
                    {/* View Mode Switch */}
                    <div className="flex items-center gap-1 bg-zinc-900/50 p-1 rounded-lg ml-2 border border-white/5">
                        <button
                            onClick={() => setViewMode('grid')}
                            className={`flex items-center gap-2 rounded-md px-3 py-1.5 text-[10px] font-bold transition-all ${viewMode === 'grid' ? 'bg-zinc-800 text-white shadow-sm' : 'text-zinc-500 hover:text-white'}`}
                        >
                            <LayoutGrid size={12} />
                            표준 그리드
                        </button>
                        <button
                            ...
                            className={`flex items-center gap-2 rounded-md px-3 py-1.5 text-[10px] font-bold transition-all ${viewMode === 'pixel' ? 'bg-amber-500 text-black shadow-lg shadow-amber-500/20' : 'text-zinc-500 hover:text-white'}`}
                        >
                            <Monitor size={12} />
                            라이브 비주얼
                        </button>
                ...
                <div className="flex items-center gap-6">
                    ...
                    <Link
                        ...
                        className="flex items-center gap-2 text-xs font-bold text-yellow-500 hover:text-yellow-400 transition-colors px-3 py-1.5 rounded-lg border border-yellow-500/30 bg-yellow-500/10 hover:bg-yellow-500/20"
                    >
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                <main className="flex flex-1 flex-col overflow-hidden relative">
                    ...
                    {/* Team Grid Layer or Pixel Art Layer */}
                    <div className="flex-1 overflow-y-auto relative">
                      ...
                      `}
                            ...
                            <div className={`w-full h-full animate-in fade-in zoom-in-95 duration-700 overflow-hidden ${isVisualFullscreen ? 'p-0' : 'p-6'}`}>
                                <div className={`w-full h-full overflow-hidden border border-white/5 shadow-2xl transition-all duration-500 ${isVisualFullscreen ? 'rounded-none border-none' : 'rounded-[2rem]'}`}>
                                    <div className="relative w-full h-full">
                                        ...
                                        <button
                                            ...
                                            className="absolute bottom-6 right-6 z-[60] bg-black/80 hover:bg-black text-white p-3 rounded-full border border-white/10 shadow-lg transition-all group"
                                            title={isVisualFullscreen ? "최소화" : "전체 화면"}
                                        >
                                            <Monitor size={20} className={isVisualFullscreen ? 'text-amber-500' : 'text-white'} />
                                            <span className="absolute right-full mr-3 whitespace-nowrap bg-black px-2 py-1 rounded text-[10px] font-bold opacity-0 group-hover:opacity-100 transition-opacity">
                                                {isVisualFullscreen ? "콘솔로 돌아가기" : "전체 화면으로 보기"}
                                            </span>
                                        </button>
                                    </div>
                                </div>
                    ...
                    {!(viewMode === 'pixel' && isVisualFullscreen) && (
